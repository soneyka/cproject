...
public class SearchController {
    ...
    public StringBuffer searchQ(@RequestBody SearchModel search) throws IOException {
        ...
        if (response.getStatusLine().getStatusCode() == 401) {
            String error = "Сервер не ответил должным образом. Пожалуйста, проверьте токен авторизации spotify (https://developer.spotify.com/web-api/console/get-search-item/).";
            logger.error(error);
